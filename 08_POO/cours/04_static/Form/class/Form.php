...
class Form
{
  ...
  {
    // self:: appelle un élément static de la classe (methode ou propriete)
  ...
  public static function select($name, $options, $selected = null)
  {
    ...
    // on verifie si on a un tableau associative ou indexé ( en verifiant si les cles sont comprises entre 0 et la fin du tableau)
    $isAssociative = (array_keys($options) !== range(0, count($options) - 1));

    // ATTENTION FONCTIONNE QU'A PARTIR DE PHP8 : array_is_list() verifie si on a un tableau index (ici on inverse (!) le résultat pour que cela fonctionne avec notre ternaire)
