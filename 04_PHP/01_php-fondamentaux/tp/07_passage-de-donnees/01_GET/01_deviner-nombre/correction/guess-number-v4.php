...
function winVerif(int $number, int $guessNumber) : string {
  if($number){
...
if(isset($_GET['number'])){
  $result = winVerif((int)$_GET['number'], $guessNumber);
}
...
<div class="container pt-5 w-50">
    ...
    <?php if(isset($result)) : ?>
      <div class="alert alert-<?= $result === 'Bravo!' ? 'success' : 'warning'?>" role="alert">
        <?= $result ?>
      </div>
    <?php endif; ?>
    ...
    <form class="row g-3" method="get">
      <div class="col-auto">
        ...
        <!-- on fait une ternaire dans le placeholder afin d'afficher la saisie precedente de l'utilisateur ou alors pour la premiere fois on affichera le message 'Saisir un nombre' -->
